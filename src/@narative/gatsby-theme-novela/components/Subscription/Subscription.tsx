...
const Form = styled.form<{ hasError: string }>`
  ...
  &::after {
    ...
    left: 21px;
    top: 10px;
    color: ${p => (p.hasError ? p.theme.colors.error : '#A92FF')};
...
const Button = styled.button<{ hasError: string; subscribed: boolean }>`
  ...
  height: 38px;
  border: 1px solid
    ${p => (p.hasError ? p.theme.colors.error : '#1A92FF')};
  color: ${p => (p.hasError ? p.theme.colors.error : '#1A92FF')};
  background: ${p => (p.subscribed ? '#1A92FF' : "transparent")};
  font-weight: 600;
  border-radius: 35px;
  ...
  &:hover {
    background: ${p =>
      p.hasError ? p.theme.colors.error : '#1A92FF'};
